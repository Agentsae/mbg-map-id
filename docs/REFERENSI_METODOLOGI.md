...
| 6 | **Indeks Kebutuhan Mobilitas** dari 3 proksi: proporsi usia rentan (lansia + balita), kepadatan POI kebutuhan harian, **proporsi penduduk usia sekolah 5–19** (`compute_indeks_kebutuhan_mobilitas()`; PRD Bab 7.2) | Jiao & Dillivan (2013) daftar *transit-dependent population* (memasukkan penduduk di bawah usia mengemudi / *under 16* dan lansia); Currie (2010) *transit needs index* — komponen "populasi usia < 18"; Litman *Evaluating Transportation Equity*, VTPI | Kelompok dengan mobilitas non-diskresioner & tanpa opsi menyetir sendiri = paling bergantung transit. Anak usia sekolah (SD–SMA) = populasi **di bawah usia mengemudi** dengan trip harian rutin ke sekolah → proksi *transit-dependent* yang punya data spasial (per-kelurahan, DKB Semester I 2026). | **Ganti 2026-09-06** (migration `026`): komponen ketiga `rasio_tanpa_kendaraan` → `proporsi_usia_sekolah`. Alasan: rasio tanpa-kendaraan tak tersedia per-kelurahan (Susenas Kota Bekasi 2025 hanya angka kota: 93,05% RT punya aset transportasi) → dulu fallback 0,5 seragam. Difabel tetap belum terdata; "usia rentan" = lansia + balita saja. Detail: `VALIDASI_BOBOT_AHP.md` Bagian 0A. |
...
- Currie, G. "Quantifying spatial gaps in public transport supply based on social needs." *Journal of Transport Geography* 18(1), 2010, pp. 31–41. *(transit needs index — komponen populasi di bawah usia mengemudi / usia < 18 sebagai proksi transit-dependent; dasar penggantian komponen ke `usia_sekolah` 2026-09-06.)*
...
4. **Belum ada studi construct validity formal** — belum ada pembuktian
   ...
   bagian 5.
5. ~~**`rasio_tanpa_kendaraan`** (komponen Indeks Kebutuhan Mobilitas): data
   BPS/Susenas belum tersedia → semua grid pakai nilai netral 0,5.~~ **Diperbaiki
   2026-09-06:** komponen ketiga diganti ke `proporsi_usia_sekolah` (umur 5–19,
   bobot 0,40) yang punya data per-kelurahan riil dan dinormalisasi min-max
   lintas grid. Pilihan usia sekolah sebagai proksi transit-dependent bersandar
   pada Currie (2010) & Jiao & Dillivan (2013); pemetaan pita umur 5–19 ke
   "usia sekolah SD–SMA" dan bobot 0,40 tetap keputusan tim (lihat
   `VALIDASI_BOBOT_AHP.md` Bagian 0A).
