...
**`penduduk.geom`** (titik). Dicek langsung: **56 baris REAL `penduduk` (DKB Semester I 2026 — Ditjen Dukcapil Kemendagri,
...
> **SUPERSEDED 2026-09-03:** bobot 13 baris ini sudah diturunkan ulang lewat sesi AHP pairwise
> Saaty **formal** (CR: CAI 0,0226 / TDI_MOBILITAS 0,0000 / EQUITY 0,0457, semua < 0,1) via
> `018_konfigurasi_bobot_ahp_final.sql` + recompute skor (019). Paragraf ini = status per 27 Agu.
...
`data-ai-analyst` upload 56 baris `penduduk` (DKB — Data Konsolidasi Bersih — Semester I 2026,
Ditjen Dukcapil Kemendagri, total 2.607.248 jiwa, `etl/load_penduduk.py`) — semua 56 kelurahan cocok ke `batas_administrasi`
...
| Jumlah penduduk + proporsi lansia/balita per kelurahan | ✅ **SELESAI 27 Agu** — 56 baris asli di tabel `penduduk`, sumber **DKB (Data Konsolidasi Bersih) Semester I 2026 — Ditjen Dukcapil Kemendagri** (✅ **RESOLVED 2026-09-03**: migration `023_penduduk_sumber_relabel.sql` diterapkan — seluruh 56 baris `penduduk` kini berlabel `'DKB Semester I 2026 - Ditjen Dukcapil Kemendagri'` (verifikasi: 56 baris, count tidak berubah, 0 baris dengan label lama `'Disdukcapil Kota Bekasi - DKB Semester 1 2026'`). `SUMBER_LABEL` di `etl/load_penduduk.py` sudah cocok dengan wording kanonik ini, jadi re-run ETL akan kena idempotency guard, bukan menyisipkan duplikat). 56/56 kelurahan cocok ke `batas_administrasi` (RBI), total 2.607.248 jiwa, tidak ada warning cross-validation (PRODUKTIF_NON = JUMDUK di semua baris). 24 baris sintetis lama dipertahankan berdampingan (FK ke 6 kelurahan dummy, masih dipakai testing RPC `simulate_new_stop`) — additive, bukan replace. **✅ RESOLVED 2026-08-28**: DKB Semester I 2026 (2.607.248 jiwa) diadopsi sebagai angka kanonik tim; PRD Bab 1 diperbarui, selisih vs proposal lama (2.595.927) jadi errata di laporan akhir. Sumber tunggal: `etl/data/demografi/profil_kota_kanonik.json`. Lihat catatan RESOLVED di bawah. | `n_kepadatan` CAI (bobot AHP 0,329 — ex aequo terbesar dgn `jarak_inv`, lihat migration 018), TDI, Equity | `etl/load_penduduk.py --excel etl/data/demografi/DAK_SEMESTER_1_TAHUN_2026_REV01.xlsx` | Selesai |
| ~~Rasio rumah tangga tanpa kendaraan pribadi~~ | **DIHAPUS dari model 2026-09-06** — komponen ketiga Indeks Kebutuhan Mobilitas diganti ke `usia_sekolah` (proporsi penduduk umur 5–19, migration `026`). Rasio tanpa-kendaraan tak tersedia per-kelurahan (Susenas Kota Bekasi 2025 hanya angka kota: 93,05% RT punya aset transportasi) → tak perlu dicari lagi. | — | — | Tidak dipakai lagi; lihat baris `proporsi usia sekolah` di bawah |
| Proporsi penduduk usia sekolah 5–19 per kelurahan | ✅ **SIAP (2026-09-06)** — diturunkan dari sheet `JUMDUK_KELUMUR` file DKB yang sama (pita 05-09 + 10-14 + 15-19 ÷ jumlah_penduduk). Dry-run `load_penduduk.py`: 56/56 kelurahan terisi, rentang 0,2223–0,2886 (median 0,2466). Kolom `penduduk.proporsi_usia_sekolah` (migration `026`, belum di-`db push`). | TDI (Indeks Kebutuhan Mobilitas, bobot 0,40) | DKB Semester I 2026 — Ditjen Dukcapil Kemendagri (file sama) | `python etl/load_penduduk.py --excel ... --update-existing` (setelah `db push` 026) |
...
| Data | Status | Index | Cara mendapatkan | Sebelum 13 Sep? |
|---|---|---|---|---|
| Bobot AHP final — CAI | ✅ **SELESAI 2026-09-03** — sesi AHP pairwise Saaty **FORMAL**. 4 baris `konfigurasi_bobot`: `kepadatan=0.3290, jarak_inv=0.3290, volume=0.2002, survei=0.1418`, `consistency_ratio=0.0226` (< 0,1). Di-set via `018_konfigurasi_bobot_ahp_final.sql` (menggantikan draft `004` + worksheet mentor `009`). Recompute skor CAI menyusul via ETL. Matriks pairwise mentah: placeholder di `docs/VALIDASI_BOBOT_AHP.md` Bagian 0.5 | CAI | Sesi AHP pairwise formal | Selesai (rollout DB dituntaskan `data-ai-analyst`) |
| Bobot AHP final — TDI (Indeks Kebutuhan Mobilitas) | ✅ **SELESAI 2026-09-03**, **komponen ke-3 diganti 2026-09-06** — 3 baris `konfigurasi_bobot`: `usia_rentan=0.2000, poi_harian=0.4000, usia_sekolah=0.4000` (dulu `tanpa_kendaraan`; diganti migration `026_tdi_mobilitas_usia_sekolah.sql` — bobot & CR 0.0000 tetap, matriks 3×3 di `VALIDASI_BOBOT_AHP.md` Bagian 0A). Bobot 0.40/0.35/0.25 dari worksheet mentor `009` sudah digantikan `018` lebih dulu. `compute_tdi()` perlu di-rerun untuk 2.607 grid setelah `db push` 026 — lihat Kategori E | TDI | Sesi AHP pairwise formal + keputusan tim 2026-09-06 | Migration `026`/`027` ditulis, belum `db push` |
| Bobot AHP final — Equity Index | ✅ **SELESAI 2026-09-03 (bobot AHP formal)** — 6 baris `konfigurasi_bobot`: `aksesibilitas_inv=0.3076, kepadatan=0.1538, usia_rentan=0.1513, akses_pendidikan=0.1260, akses_kesehatan=0.1353, akses_kerja=0.1260`, `consistency_ratio=0.0457` (< 0,1). Di-set via `018_konfigurasi_bobot_ahp_final.sql` (menggantikan worksheet mentor `009` yang memakai 0.30/0.15/0.20/0.15/0.10/0.10). `compute_equity_index()` di-rerun + `kelompok_terdampak`/`rekomendasi_intervensi` diisi ulang via `019_equity_kelompok_rekomendasi_refill_ahp.sql` — lihat Kategori E | Equity | Sesi AHP pairwise formal | Selesai (rollout DB dituntaskan `data-ai-analyst`) |
...
2. ~~**Segera**: konfirmasi status sesi AHP dengan mentor (CAI/TDI/Equity)~~ ✅ **SELESAI 2026-09-03** — sesi AHP pairwise Saaty formal, bobot final + CR (< 0,1 ketiganya) di `konfigurasi_bobot` via migration 018/019
...
6. ~~**Terima fallback**: rasio tanpa kendaraan (Susenas) tetap 0.5 kalau tidak sempat~~ ✅ **DIGANTI 2026-09-06** — komponen ketiga Indeks Kebutuhan Mobilitas kini `proporsi_usia_sekolah` (umur 5–19) dari DKB, bukan lagi fallback konstan (migration `026`)
