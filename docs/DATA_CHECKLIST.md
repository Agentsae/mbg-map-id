...
| Bobot AHP final — TDI (Indeks Kebutuhan Mobilitas) | ✅ **SELESAI 2026-09-03 (bobot AHP formal)** — 3 baris `konfigurasi_bobot`: `usia_rentan=0.2000, poi_harian=0.4000, tanpa_kendaraan=0.4000`, `consistency_ratio=0.0000` (matriks 3×3 transitif penuh). Di-set via `018_konfigurasi_bobot_ahp_final.sql` (menggantikan worksheet mentor `009` yang memakai 0.40/0.35/0.25). `compute_tdi()` di-rerun untuk 2.607 grid dengan bobot baru — lihat Kategori E | TDI | Sesi AHP pairwise formal | Selesai (rollout DB dituntaskan `data-ai-analyst`) |
