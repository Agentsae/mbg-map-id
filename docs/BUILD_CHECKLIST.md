...
### B-bis. Model harus MENGUSULKAN lokasi, bukan cuma menilai lokasi yang disurvei
- [x] **Generator usulan halte berbasis model (2026-09-07)** — gap metodologis terbesar sebelum ini: PRD Bab 1.1 menjanjikan jawaban "di titik mana pengembangan transit memberi dampak aksesibilitas terbesar", tapi satu-satunya daftar usulan yang ada adalah 19 `titik_kandidat` hasil survei yang diurut CAI. Tidak ada langkah yang menurunkan kandidat halte dari keluaran TDI/Equity — pertanyaan juri "kenapa 19 titik ini?" hanya bisa dijawab "karena kami survei di sana".
  - Baru: `etl/generate_usulan_halte_model.py` + tabel `usulan_halte_model` (`supabase/migrations/028_usulan_halte_model.sql`, sudah `db push`). **25 baris terisi.**
  - Pipeline: 2.607 sel `grid_analisis` → 1.503 sel transit desert (`skor_tdi` > 0,6, ambang yang sama dengan `TRANSIT_DESERT_THRESHOLD` di `Dashboard.jsx`/`DataLaporan.jsx` dan migration `022`) → filter centroid ≥ 400 m dari `halte_eksisting` REAL (`DUMMY-HLT-*` dikecualikan; 400 m = catchment ITDP di `003_simulate_new_stop.sql`) → de-klaster greedy jarak minimum 800 m antar-usulan (`AMBANG_NIHIL_M`) → 25 titik → RPC `simulate_new_stop` untuk `penduduk_terlayani_400m/800m` → ranking primer populasi terlayani, sekunder `skor_tdi_sel`.
  - **Pemisahan tegas yang harus dijaga di UI & narasi:** `titik_kandidat` = kandidat **tervalidasi lapangan** (Survey Activities); `usulan_halte_model` = **usulan model spasial, BELUM disurvei lapangan**. Jangan digabung dalam satu daftar tanpa label.
  - **Sengaja tidak menghitung CAI** untuk titik model: kriteria volume CAI berasal dari traffic counting lapangan yang titik model tidak punya → mengisinya = mengarang angka (melanggar prinsip inti CLAUDE.md). Juga **tidak** di-insert ke `titik_kandidat` supaya normalisasi min-max CAI 19 kandidat survei tidak bergeser diam-diam.
  - Sanity check run 2026-09-07 LOLOS: 0 NULL, 0 usulan berpopulasi nol, jarak terdekat ke halte real 1.117 m, jarak antar-usulan terdekat 848 m, semua 25 titik jatuh di dalam poligon kelurahan RBI (8 kecamatan). `penduduk_terlayani_800m` 6.945–61.728 (median 30.193). RLS: publik read-only terverifikasi (anon SELECT 25 baris OK, anon INSERT ditolak).
- [ ] **Belum di-surface di frontend** — layer peta + bagian Rekomendasi/Dashboard untuk `usulan_halte_model` belum dibuat (tugas `webgis-developer`). Wajib diberi label "usulan dari model spasial — belum disurvei lapangan".

### E. Terjadwal
- [~] **Fase 4 — swap data asli**: MAYORITAS TUNTAS, lihat detail di Fase 4 di bawah (migration `018`/`019`/`024` sudah `db push`). Sisa: re-validasi kecepatan dengan volume data asli. **Update 2026-09-07:**
  - Batch traffic counting KND-010..KND-023 (13 baris) sudah di `titik_kandidat` dengan **koordinat survei RIIL** (surveyor Galuh, 17 Agu) — bukan estimasi (catatan lama "koordinat masih estimasi" usang).
  - **CAI `jarak_fasilitas_m` kini dihitung dari `geom` via `ST_Distance` ke POI** (bukan lagi kolom Excel `jarak_transit_terdekat_m` yang semantiknya tidak konsisten antar-batch) — `attach_cai_features_titik_kandidat.py`, applied. `n_jarak_inv` sekarang rentang penuh 0–1 di kedua batch.
  - **CAI kriteria "skor survei kondisi halte" jadi N/A untuk `titik_kandidat`** (bukan 0): dikeluarkan dari WLC, 3 bobot AHP sisanya direnormalisasi (keputusan tim 2026-09-07, `exclude_criteria=['survei']`). `skor_cai.n_survei`/`bobot_survei` = NULL. Ranking 19 kandidat tidak berubah (rescale seragam ×1,165). Lihat `docs/VALIDASI_BOBOT_AHP.md` + CLAUDE.md bagian CAI.
