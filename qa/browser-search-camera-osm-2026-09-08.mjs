// Camera-move verification with OSM raster fallback basemap (so map 'load' fires
// and App.onMapReady sets mapInstance). vite preview on :4322.
...
const chrome = spawn(CHROME, ['--headless=new', `--remote-debugging-port=${PORT}`, '--no-first-run',
  ...
  '--user-data-dir=' + process.env.TEMP + '/qa-chrome-osm', 'about:blank'], { stdio: 'ignore' })
...
console.log('map state (OSM fallback):', JSON.stringify(st))
...
// SearchBar prop check via minification-proof: find fiber whose memoizedProps has both `halte` and `mapInstance` keys
const sbProp = await ev(`(() => {
...
console.log('SearchBar.mapInstance prop:', JSON.stringify(sbProp))
...
async function typeAndSelect(text, rowRe) {
  ...
  return { text, rows, clicked, before, after, changedAt }
}
...
console.log('\n-- Halte (point -> flyTo zoom 16) --')
console.log(JSON.stringify(await typeAndSelect('STISIP', '/Halte/'), null, 1))
