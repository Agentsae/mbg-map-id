...
- **Composite Accessibility Index (CAI)** — Weighted Linear Combination dari 4 kriteria, tiap kriteria dinormalisasi 0–1 sebelum dikalikan bobot (supaya kontribusi per kriteria bisa ditelusuri per lokasi saat diklik — PRD final Bab 7.3). **Sejak 2026-09-10 CAI dihitung sebagai permukaan grid 300 m di `grid_analisis` (unit spasial sama dengan TDI), bukan lagi hanya di 19 titik survei — lihat Catatan (2026-09-10) di bawah.** Empat kriteria & bobotnya:
  ...
  **Catatan (2026-09-10, keputusan tim — deviasi metodologi yang DISENGAJA, sekelas swap `tanpa_kendaraan`→`usia_sekolah` 2026-09-06):** CAI tidak lagi hanya dihitung di 19 `titik_kandidat` lalu ditampilkan lewat *nearest-neighbour lookup*. Sejak 2026-09-10 CAI juga dihitung sebagai **permukaan grid 300 m** di `grid_analisis`, lewat migration `030_skor_cai_grid.sql` + `etl/compute_cai_grid.py` (nilai R² regresi & nama field RPC: rujuk migration `030`, bukan dokumen ini). Alasan: memperluas survei lapangan sebelum submission 13 Sep tidak feasible; grid membuat CAI konsisten-spasial dengan TDI dan menghapus perilaku menyesatkan "skor titik survei terdekat ditampilkan seolah skor lokasi yang diklik" (ambang interim *nearest-point* 2000 m yang sempat dipakai lebih awal 2026-09-10 dicabut — klik di luar grid berpenduduk kini balas "di luar cakupan analisis", sama seperti TDI). **Bobot & AHP TIDAK berubah** (tetap `konfigurasi_bobot` `nama_index='CAI'`: 0,329 / 0,329 / 0,200 / 0,142; CR 0,0226) — hanya unit spasial & cara kriteria volume diisi yang berubah. Normalisasi min-max kini lintas seluruh sel grid berpenduduk. Perhitungan per sel bersifat **hibrida**:
    - **Kepadatan penduduk:** nilai dasymetric per sel (sumber tak berubah).
    - **Jarak ke fasilitas umum (inverse):** `ST_Distance` centroid sel → POI fasilitas terdekat (OSM sekolah/faskes/kerja) — bisa dihitung di semua sel.
    - **Volume penumpang transit — HIBRIDA:** sel yang memuat / ≤ 300 m dari titik survei → pakai hasil cacah Traffic Counting 2 jam terukur (`total_aktivitas`). Sel lain → **estimasi** dari regresi linear volume terukur atas kepadatan penduduk (fit pada 19 titik survei), **di-clip ke rentang teramati (tanpa ekstrapolasi)**, ditandai `cai_volume_estimasi = true`. **Pengungkapan jujur WAJIB di mana pun angka ini muncul (panel klik peta, Export Report, narasi AI):** nilainya estimasi hasil pembandingan dengan lokasi survei (regresi kepadatan↔volume atas 19 titik survei), **bukan hasil cacah lapangan di lokasi itu**.
    - **Skor survei kondisi halte:** dipakai per sel HANYA bila ada halte eksisting tersurvei (punya Form Kondisi Halte) ≤ 400 m; jika tidak → N/A dan 3 bobot AHP sisanya direnormalisasi jadi Σ = 1 untuk sel itu — mekanisme sama persis dengan `titik_kandidat` sejak 2026-09-07 (≈ 0,3834 / 0,3834 / 0,2333).

    Grid CAI bersifat **aditif** — tabel `skor_cai` berbasis titik dan ranking 19 kandidat survei **tidak dihapus**: tetap rujukan kanonik CAI untuk ke-19 kandidat **tervalidasi lapangan** dan tetap basis normalisasi min-max `usulan_halte_model`. Prinsip inti tak berubah: **AI tetap hanya lapisan interpretasi** (regresi kepadatan↔volume adalah model spasial deterministik yang koefisien & R²-nya tersimpan di migration `030`, bukan LLM), dan **setiap skor sel tetap bisa ditelusuri** — RPC grid mengembalikan rincian kontribusi tiap kriteria + flag `cai_volume_estimasi`, bukan angka tunggal.
...
| Composite Accessibility Index & TDI | Klik lokasi di peta → tampilkan skor + **rincian kontribusi tiap kriteria** (bukan angka tunggal tanpa penjelasan). Sejak 2026-09-10 CAI dibaca dari **sel grid 300 m** (`grid_analisis`) via RPC baru (migration `030`) — rincian per kriteria tetap dikembalikan (kriteria Bab 8 tetap terpenuhi); klik di luar grid berpenduduk balas "di luar cakupan analisis". Bila `cai_volume_estimasi = true`, panel WAJIB menandai komponen volume sebagai estimasi regresi kepadatan↔volume atas 19 titik survei, **bukan cacah lapangan**. |
